...
if send:
    if uploaded_audio:
        ...
        with st.spinner("Transcribing your audio..."):
            try:
                ...
                if transcribe_response.status_code == 200:
                    ...
                    container.error(f"Transcription Error: {transcribe_response.json().get('detail')}")
                    transcription = ""
        ...
        if transcription:
            ...
            st.session_state.messages.append({"role": "user", "content": transcription, "audio": uploaded_audio})
            ...
            with st.spinner("AI is responding..."):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        container.error(f"Chat Error: {response.json().get('detail', 'Unknown error')}")
                        reply = ""
                except Exception as e:
                    # tb.print_exc()
            ...
            with st.spinner("Converting AI response to speech..."):
                try:
                    generate_response = requests.post(
                        ...
                        json={"text": reply, "voice": "Male", "checkpoint": "states_6000"},
                    )
                    if generate_response.status_code == 200:
                        ...
                        container.error(f"TTS Error: {generate_response.json().get('detail')}")
                except Exception as e:
